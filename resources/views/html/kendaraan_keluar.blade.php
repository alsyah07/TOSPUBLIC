            ...
            <div class="content-header row">
                <div class="content-header-left col-md-9 col-12 mb-2">
                    <div class="row breadcrumbs-top">
                        <div class="col-12">
                             @if($menu->create==1)
                            <h2 class="content-header-title float-start mb-0"><a class="btn btn-success" href="/importkendaraankeluar">Import</a></h2>
                            @endif
                            <h2 class="content-header-title float-start mb-0">Data Kendaraan Keluar</h2>
                ...
                <div class="content-header-right text-md-end col-md-3 col-12 d-md-block d-none">
                    <button type="button" class="btn btn-primary registerusers" >+ Tambah Kendaraan Keluar</button>
                </div>
                @endif
                 @if(session('notif'))
...
<table id="example" class="table table-responsive table-striped" style="width:100%; overflow: auto; overflow: auto;white-space:nowrap;">
        ...
        <tbody>
            @foreach($kendaraan_keluar as $val)
            <tr>
                ...
                {{-- @if($val->status_spinoam==1)
                <td>Valid</td>
                @else
                 <td>Expired</td>
                @endif        
                @if($val->status_eblue==1)
                <td>Valid</td>
                @else
                 <td>Expired</td>
                @endif --}}
                <td>{{ $val->tipe_kendaraan }}</td>
                <td><a href="/fotokendaraan/{{ $val->no_kendaraan }}/{{ $val->jam }}/keluar" class="btn btn-secondary btn-sm view_foto" id="{{ $val->no_kendaraan }}">Lihat Foto</a></td>
                <td>
                    @if($menu->update==1)
                    <button  class="btn btn-sm btn-success edit_data" style="width:40%;" id="{{ $val->id_kendaraan_keluar }}"><span style="font-size: 10px;">Edit </span><i class="fas fa-edit"></i></button>  
                    @endif
                    @if($menu->delete==1)
                    <a style="width:50%;" href="#"  data-bs-toggle="modal" data-bs-target="#moddelete" onclick="confdelete({{ $val->id_kendaraan_keluar }})" class="btn btn-sm btn-danger"><span style="font-size: 10px;"> Hapus </span> <i class="fas fa-trash"></i></a>
                    @endif
                </td>
            </tr>
            @endforeach
    ...
    <form class="form" method="post" action="/simpankendaraankeluar" enctype="multipart/form-data" > 
        @csrf()
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                <div class="modal-header" style="background-color: #17abb0;">
                    <h4 class="modal-title" id="myModalLabel16" style="color: white">Form Kendaraan Keluar</h4>
                                    ...
                                    <div class="col-md-6">
                                       ...
                                       {{--  <div class="form-group mb-1">
                                        ...
                                        <div class="form-group mb-1">
                                            ...
                                            <div class="row" style="margin-top: 20px;">
                                                <div class="col-md-2">
                                                    <input type="radio" class="form-check-input" id="eblue1"  name="eblue" value="1">
                                                    <span>Valid</span>
                                                </div>
                                                <div class="col-md-2">
                                                    <input type="radio" class="form-check-input" id="eblue2"  name="eblue" value="0">
                                                    <span>Expired</span>
                                                </div>
                                            </div>
                                        </div>
    ...
    <form class="form" method="post" action="/updatekendaraankeluar"> 
        @csrf()
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body px-sm-5 mx-50 pb-5">
                                ...
                                <!-- form -->
                                <form method="post" action="/deletekendaraankeluar">
                                    @csrf
                                    <div class="col-12">
    ...
    $(document).ready(function () {
        ...
        $(document).on('click','.edit_data',function(){
            var idubh = $(this).attr('id');
                $.ajax({
                  url :'/api/updatedatakendaraankeluar/'+idubh+'/'+ {{ Auth::user()->id }},
                  method : 'GET',
                  dataType : 'json',
                  success:function(data){
                    console.log('datajeke',data);
                      ...
                      $('#masa_berlaku_kendaraanup').val(data.data.masa_berlaku_kendaraan);
                       $('#id').val(data.data.id_kendaraan_keluar)
